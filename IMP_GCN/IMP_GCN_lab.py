...
class IMP_GCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.ua_embeddings, self.ia_embeddings, self.ia_embeddings_sub1, self.ia_embeddings_sub2, self.A_fold_hat_group_filter, self.user_group_embeddings = self._create_imp_gcn_embed()
        """
        *********************************************************
        ...
        self.pos_i_g_embeddings_sub1 = tf.nn.embedding_lookup(self.ia_embeddings_sub1, self.pos_items)
        self.pos_i_g_embeddings_sub2 = tf.nn.embedding_lookup(self.ia_embeddings_sub2, self.pos_items)
        self.neg_i_g_embeddings = tf.nn.embedding_lookup(self.ia_embeddings, self.neg_items)
        ...
        *********************************************************
        ...
        """
        self.ssl_loss = self.calc_ssl_loss(self.pos_i_g_embeddings_sub1,self.pos_i_g_embeddings_sub2)
    ...
    def _create_imp_gcn_embed(self):
        ...
        all_embeddings_sub1, all_embeddings_sub2 = [],[]
        ego_embeddings_f = []
        for k in range(1, self.n_layers):
          # subgraph마다 convolution operation
            for g in range(0,self.group):
                ...
                if k == 1:
                    ego_embeddings_f.append(side_embeddings)
                else:
                  ego_embeddings_f[g] = side_embeddings
            # ego_embedding_f : Layer K에서 각 subgraph의 임베딩
            all_embeddings_sub1.append(ego_embeddings_f[0])
            all_embeddings_sub2.append(ego_embeddings_f[1])
        ...
        if (epoch + 1) % 10 != 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                perf_str = 'Epoch %d [%.1fs]: train loss==[%.5f=%.5f + %.5f + %.5f]' % (
                    epoch, time() - t1, loss, bpr_loss, emb_loss, ssl_loss)
                print(perf_str)
            continue
